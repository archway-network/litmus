...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libarchway").join("artifacts");

    let lib_name = "archway";
    ...
    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libarchway")
            .join("artifacts")
            .join("libarchway.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libarchway");
    ...
    if env::var("PREBUILD_LIB") == Ok("1".to_string()) {
        build_libarchway(prebuilt_lib_dir.join(lib_filename));
    }
    ...
    if std::fs::metadata(&out_dir_lib_path).is_err()
        || env::var("ARCHWAY_TUBE_DEV") == Ok("1".to_string())
    {
        build_libarchway(out_dir_lib_path);
    }
    ...
    if env::var("PROFILE").unwrap() == "debug" {
        ...
        for entry in std::fs::read_dir(out_dir.clone()).unwrap() {
            ...
            if path.is_file() {
                ...
                if file_name.starts_with("libarchway") {
...
fn build_libarchway(out: PathBuf) {
    ...
    let tidy_status = Command::new("go")
        .current_dir(manifest_dir.join("libarchway"))
        .arg("mod")
        .arg("tidy")
        .spawn()
        .unwrap()
